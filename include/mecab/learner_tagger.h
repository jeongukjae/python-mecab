...
class LearnerTagger {
 public:
  ...
  explicit LearnerTagger() : tokenizer_(0), path_allocator_(0), feature_index_(0), begin_(0), end_(0), len_(0) {}
 ...
 protected:
  ...
  bool connect(size_t pos, LearnerNode* _rNode) {
    for (LearnerNode* rNode = _rNode; rNode; rNode = rNode->bnext) {
      for (LearnerNode* lNode = end_node_list_[pos]; lNode; lNode = lNode->enext) {
  ...
  bool initList() {
    ...
    std::fill(end_node_list_.begin(), end_node_list_.end(), reinterpret_cast<LearnerNode*>(0));

    begin_node_list_.resize(len_ + 2);
    std::fill(begin_node_list_.begin(), begin_node_list_.end(), reinterpret_cast<LearnerNode*>(0));
...
class EncoderLearnerTagger : public LearnerTagger {
 public:
  bool open(Tokenizer<LearnerNode, LearnerPath>* tokenizer,
  ...
  bool read(std::istream* is, std::vector<double>* observed) {
    ...
    for (size_t i = 0; i < corpus.size(); ++i) {
      LearnerNode* rNode = 0;
      for (LearnerNode* node = begin_node_list_[pos]; node; node = node->bnext) {
        if (corpus[i]->stat == MECAB_EOS_NODE || node_cmp_eq(*(corpus[i]), *node, eval_size_, unk_eval_size_)) {
  ...
  int eval(size_t* crr, size_t* prec, size_t* recall) const {
    int zeroone = 0;
...
class DecoderLearnerTagger : public LearnerTagger {
 public:
  bool open(const Param& param) {
    ...
    CHECK_DIE(tokenizer_->open(param));
  ...
  bool parse(std::istream* is, std::ostream* os) {
    ...
    for (LearnerNode* node = end_node_list_[0]->next; node->next; node = node->next) {
