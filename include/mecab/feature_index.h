...
class FeatureIndex {
 public:
  ...
  bool buildUnigramFeature(LearnerPath* path, const char* ufeature) {
  ...
  bool buildBigramFeature(LearnerPath* path, const char* rfeature, const char* lfeature) {
  ...
  const char* strdup(const char* p) {
  ...
  static bool convert(const Param& param, const char* txtfile, std::string* output) {
  ...
  static bool compile(const Param& param, const char* txtfile, const char* binfile) {
    std::string buf;
  ...
  explicit FeatureIndex() : feature_freelist_(8192 * 32), char_freelist_(8192 * 32), maxid_(0), alpha_(0) {}
 ...
 protected:
  ...
  const char* getIndex(char** p, char** column, size_t max) {
    ++(*p);
...
class EncoderFeatureIndex : public FeatureIndex {
 public:
  bool open(const Param& param) { return openTemplate(param); }
  void close() {
    dic_.clear();
  ...
  bool reopen(const char* filename, const char* dic_charset, std::vector<double>* alpha, Param* param) {
    close();
...

    {
      ...
      if (it != feature_cache_.end()) {
        ...
        if (!buildBigramFeature(path, rfeature1.c_str(), lfeature2.c_str()))
          return false;
        feature_cache_.insert(std::pair<std::string, std::pair<const int*, size_t>>(
            key, std::pair<const int*, size_t>(path->fvector, 1)));
      }
    }
...
class DecoderFeatureIndex : public FeatureIndex {
 public:
  ...
  void clear() { feature_freelist_.free(); }
  void close() {
    mmap_.close();
    model_buffer_.clear();
    maxid_ = 0;
  ...
  const char* charset() const { return charset_; }
 ...
 private:
  ...
  bool openBinaryModel(const Param& param) {
    const std::string modelfile = param.get<std::string>("model");
    CHECK_DIE(mmap_.open(modelfile.c_str())) << "cannot open file";
