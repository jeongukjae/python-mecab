...
inline bool node_cmp_eq(const LearnerNode& node1, const LearnerNode& node2, size_t size, size_t unk_size) {
  if (node1.length == node2.length && strncmp(node1.surface, node2.surface, node1.length) == 0) {
...
inline void calc_expectation(LearnerPath* path, double* expected, double Z) {
  ...
  const double c = std::exp(path->lnode->alpha + path->cost + path->rnode->beta - Z);
