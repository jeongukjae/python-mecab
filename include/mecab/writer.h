...
class Writer {
        ...
        case '%': {  // macros
          switch (*++p) {
            ...
            case 'm':
              ...
              break;
            case 'M':
              os->write(reinterpret_cast<const char*>(node->surface - node->rlength + node->length), node->rlength);
              break;
            case 'h':
              *os << node->posid;
              ...
              switch (*++p) {
                ...
                case 'i':
                  *os << node->id;
                  break;  // node id
                case 'S':
                  os->write(reinterpret_cast<const char*>(node->surface - node->rlength + node->length),
                ...
                case 's':
                  *os << static_cast<int>(node->surface - lattice->sentence());
                  ...
                  // end position
                case 'e':
                  *os << static_cast<int>(node->surface - lattice->sentence() + node->length);
                  break;
                  // connection cost
                case 'C':
 ...
 private:
  ...
  bool writeWakati(Lattice* lattice, StringBuffer* os) const {
    for (const Node* node = lattice->bos_node()->next; node->next; node = node->next) {
      os->write(node->surface, node->length);
      *os << ' ';
    }
  ...
  bool writeNone(Lattice* lattice, StringBuffer* os) const {
    return true;  // do nothing
  }
  bool writeUser(Lattice* lattice, StringBuffer* os) const {
    if (!writeNode(lattice, bos_format_.get(), lattice->bos_node(), os)) {
      return false;
    }
    const Node* node = 0;
    for (node = lattice->bos_node()->next; node->next; node = node->next) {
      const char* fmt = (node->stat == MECAB_UNK_NODE ? unk_format_.get() : node_format_.get());
  ...
  bool writeDump(Lattice* lattice, StringBuffer* os) const {
    ...
    for (const Node* node = lattice->bos_node(); node; node = node->next) {
      ...
      *os << ' ' << node->feature << ' ' << static_cast<int>(node->surface - str) << ' '
          << static_cast<int>(node->surface - str + node->length) << ' ' << node->rcAttr << ' ' << node->lcAttr << ' '
          << node->posid << ' ' << static_cast<int>(node->char_type) << ' ' << static_cast<int>(node->stat) << ' '
          << static_cast<int>(node->isbest) << ' ' << node->alpha << ' ' << node->beta << ' ' << node->prob << ' '
          << node->cost;
  ...
  bool writeEM(Lattice* lattice, StringBuffer* os) const {
