...
class DoubleArrayImpl {
 ...
 public:
  ...
  void set_result(value_type& x, value_type r, size_t) const { x = r; }
  ...
  const void* array() const { return const_cast<const void*>(reinterpret_cast<void*>(array_)); }
  ...
  int open(const char* file, const char* mode = "rb", size_t offset = 0, size_t size = 0) {
    ...
    if (size_ != std::fread(reinterpret_cast<unit_t*>(array_), sizeof(unit_t), size_, fp))
      return -1;
    std::fclose(fp);
  ...
  int save(const char* file, const char* mode = "wb", size_t offset = 0) {
    ...
    if (!fp)
      return -1;
    if (size_ != std::fwrite(reinterpret_cast<unit_t*>(array_), sizeof(unit_t), size_, fp))
      return -1;
    std::fclose(fp);
    return 0;
  }
  ...
  int gzopen(const char* file, const char* mode = "rb", size_t offset = 0, size_t size = 0) {
    ...
    if (!gzfp)
      return -1;
    array_ = new unit_t[size_];
    if (zlib::gzseek(gzfp, offset, SEEK_SET) != 0)
      return -1;
