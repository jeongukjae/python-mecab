...
template Node* Tokenizer<Node, Path>::lookup<false>(const char*, const char*, Allocator<Node, Path>*, Lattice*) const;
template Node* Tokenizer<Node, Path>::lookup<true>(const char*, const char*, Allocator<Node, Path>*, Lattice*) const;
...
template const DictionaryInfo* Tokenizer<LearnerNode, LearnerPath>::dictionary_info() const;
...
template LearnerNode* Tokenizer<LearnerNode, LearnerPath>::lookup<false>(const char*,
                                                                         const char*,
                                                                         Allocator<LearnerNode, LearnerPath>*,
                                                                         Lattice*) const;
template bool Tokenizer<LearnerNode, LearnerPath>::open(const Param&);

template <typename N, typename P>
Tokenizer<N, P>::Tokenizer() : dictionary_info_freelist_(4), dictionary_info_(0), max_grouping_size_(0) {}
...
N* Tokenizer<N, P>::lookup(const char* begin, const char* end, Allocator<N, P>* allocator, Lattice* lattice) const {
  CharInfo cinfo;
  N* result_node = 0;
  size_t mblen = 0;
  size_t clen = 0;
