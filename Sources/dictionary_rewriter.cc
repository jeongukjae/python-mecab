...
bool RewritePattern::set_pattern(const char* src, const char* dst) {
  scoped_fixed_array<char, BUF_SIZE> buf;
  spat_.clear();
  dpat_.clear();
...
bool RewritePattern::rewrite(size_t size, const char** input, std::string* output) const {
  if (spat_.size() > size)
    return false;
...
bool RewriteRules::rewrite(size_t size, const char** input, std::string* output) const {
...
bool DictionaryRewriter::open(const char* filename, Iconv* iconv) {
  ...
  while (std::getline(ifs, line)) {
    ...
    if (line == "[unigram rewrite]") {
      ...
      switch (append_to) {
        case 1:
          append_rewrite_rule(&unigram_rewrite_, str);
          break;
        case 2:
          append_rewrite_rule(&left_rewrite_, str);
          break;
        case 3:
          append_rewrite_rule(&right_rewrite_, str);
          break;
      }
    }
  }
