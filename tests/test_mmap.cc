...
TEST(mecab_mmap, test_open_mmap_instance) {
  ...
  ASSERT_TRUE(mmap.open("../test-data/cc/file-used-in-test-mmap.txt"));
  // file content: "test content" + new line
  ASSERT_STREQ(mmap.begin(), "test content\n");
  ASSERT_EQ(mmap[3], 't');
  ASSERT_EQ(mmap.file_name(), "../test-data/cc/file-used-in-test-mmap.txt");
...
TEST(mecab_mmap, test_open_mmap_instance_with_invalid_mode) {
  ...
  ASSERT_FALSE(mmap.open("../test-data/cc/file-used-in-test-mmap.txt", "invalid"));
  std::string captured = testing::internal::GetCapturedStderr();
  EXPECT_THAT(captured,
              ::testing::HasSubstr("unknown open mode: ../test-data/cc/file-used-in-test-mmap.txt mode: invalid"));
}
